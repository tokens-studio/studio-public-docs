...
<!-- {% hint style="info" %} Integrating accessibility checks directly in your token workflow ensures your design system supports all users from the foundation up, rather than relying on post-design adjustments. {% endhint %} -->
...
<!-- {% hint style="info" %} When building design token systems, run all your primary and secondary palette colors through this node connected to a `Compare` node to ensure your action/success/error token colors remain distinguishable in all vision modes. Deuteranopia (green-blind) is the most common type of color blindness, so prioritize testing with this mode. {% endhint %} -->

### See Also
- **Contrast**: Tests the accessibility of text against background colors for WCAG compliance (see `accessibility/contrast.md`).
- **Color Scale**: Generates accessible color scales that maintain distinctiveness across vision types (see `color/scale.md`).
- **Deconstruct**: Breaks down colors into components to modify after testing for color blindness (see `color/deconstruct.md`).
- **Blend**: Helps adjust colors that have poor visibility in certain color blindness modes (see `color/blend.md`).
