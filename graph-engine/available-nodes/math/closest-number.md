...
2. Connect an array of numbers (like `[8, 16, 24, 32]`) to the "Numbers" input, or use the default `[1, 2, 3]`.
3. Connect your target value (like `22`) to the "Target" input, or use the default `2`.
4. The outputs will show the closest value (`24`), its index in the array (`2`), and the difference from your target (`2`).

![Closest Number Example](screenshot-placeholder.png)
